...
class Depop:
    def __init__(self, headless=False, profile: Optional[str] = '') -> None:
        ...
        # Last item
        self.note_file = 'note/depop.json'
    ...
    def login(self, depop_username, depop_password):
        ...
        is_success = self.se.login_with_cookies(
            is_logged_in_selector, self.cookie_file)
        if is_success:
            return True

        # accept all cookies
        self.se.find_element(
            'button[data-testid="cookieBanner__acceptAllButton"]', click=True, print_error=False)
    ...
    def get_sold_items(self):
        ...
        # accept all cookies
        self.se.find_element(
            'button[data-testid="cookieBanner__acceptAllButton"]', timeout=5, click=True, print_error=False)
    ...
    def get_order_items(self, item):
        ...
        try:
            # Shipping State
            shipping_status = self.se.find_element(
                'div[data-testid="receipt__shipping_status"]')
            assert shipping_status is not None, 'Shipping status not found'
            if shipping_status.text != 'Awaiting shipping':
                return []

            receipts_div = self.se.find_element(
                'div[class^="ReceiptsList"]', timeout=1, parent=item, click=True)
            ...
            # Order Earning
            total_earning = float(aside.select(
                'table tr')[-1].text.split('$')[1])
